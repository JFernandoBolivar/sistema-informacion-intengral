...
import { entryEmployeeAction } from "../actions/employee-actions";
...
export const FormRegisterEmployee = () => {
  ...
  const onSubmit = (values: z.infer<typeof employeeRegisterSchema>) => {
    setError(null);
  ...
  const entryEmployee = async () => {
    const response = await entryEmployeeAction(data[0]);
  ...
  return (
    <div>
      <Form {...form}>
        ...
        >
          <h1 className="text-2xl">Ingresar trabajador</h1>
              ...
              render={({ field }) => (
                <FormItem className="w-[320px]">
                  ...
                  >
                    ...
                    <SelectContent>
                      {denominacionCargoEspecifico.map((selectValue, index) => {
                        return (
                          <SelectItem key={index} value={selectValue}>
                            {selectValue}
                          </SelectItem>
      ...
      <dialog id="dialog" className="w-[90%] p-4 rounded m-auto">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={entryEmployee} className="bg-green-600">
            Confirmar
          </Button>
        </div>
      </dialog>
